...
PokemonInfoResponse pokemonInfoResponseFromJson(String str) => PokemonInfoResponse.fromJson(json.decode(str));

String pokemonInfoResponseToJson(PokemonInfoResponse data) => json.encode(data.toJson());
...
class PokemonInfoResponse {
  ...
  PokemonInfoEntity toPokemonInfo() => PokemonInfoEntity(name: name,imageUrl: sprites.other?.officialArtwork.frontDefault.toString(), color: getPokemonColor(types.first.type.name), types: types.map((e) => e.toType()).toList(),id: id);

  List<ColorPokemon> colorPokemon() => types.map((e) => ColorPokemon.parse(e.type.name)).toList();


  Map<ColorPokemon, double> get typeEffectiveness {
    final effectiveness = ColorPokemon.values.where((type) => type != ColorPokemon.unknown).map(
          (type) => MapEntry(
        type,
        colorPokemon()
            .map((colorPokemon) => colorPokemon.effectiveness[type] ?? 1.0)
...
class Species {
  ...
  SpeciesEntity toSpecies() => SpeciesEntity(name: name, url: url);
...
class Type {
  ...
  Color getColor() => getPokemonColor(type.name);

  TypeEntity toType() => TypeEntity(slot: slot,color: getPokemonColor(type.name), type: type.toSpecies());
